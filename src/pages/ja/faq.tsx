...
const FAQ_SECTIONS = [
  {
    ...
    items: [
      {
        q: "USDT（テザー）とは何ですか？",
        a: "USDTは1ドルに価値が連動したステーブルコインです。ビットコインやイーサリアムと違い価格変動がほとんどなく、決済や送金に適しています。CoinBypass（コインチャージ所）でウォンからUSDTをチャージして決済に利用できます。",
      },
      {
        q: "CoinBypass（コインチャージ所）とは何ですか？",
        a: "ウォンでUSDTなどの暗号資産をチャージしてくれるサービスを一般にCoinBypassまたはコインチャージ所と呼びます。入金すると指定したウォレットアドレスへUSDTを送ってくれます。レートと手数料を透明に公開しているところを利用するのが安全です。",
      },
      {
  ...
  return (
    <>
      <Head
        ...
        description="USDTチャージ、コイン決済、決済の迂回に関するよくある質問。CoinBypassの使い方、ネットワークの選び方、安全対策のFAQ。"
        keywords="USDTチャージFAQ, コイン決済の質問, CoinBypassの質問, 決済迂回の質問, テザーチャージFAQ"
